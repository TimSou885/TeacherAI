...
只輸出 JSON 陣列。`,
  ...
  if (params.learningObjectives?.trim()) {
    extra.push(`教學目標（出題時請對齊）：\n${params.learningObjectives.trim()}`)
  }
  if (params.keyVocabulary?.trim()) {
    extra.push(`重點詞彙（默書/詞語題請優先涵蓋）：\n${params.keyVocabulary.trim()}`)
  }
  if (params.textbookRef?.trim()) {
    extra.push(`教科書對照：${params.textbookRef.trim()}`)
  }
  const extraBlock = extra.length > 0 ? `\n${extra.join('\n\n')}\n` : ''

  const diffMap: Record<string, string> = { easy: '偏易，多基礎題', medium: '適中', hard: '偏難，可含進階題' }
  const diffNote = params.difficulty
    ? `難度：${diffMap[params.difficulty] ?? '適中'}。`
    : `難度：適合小學${grade}年級。`
