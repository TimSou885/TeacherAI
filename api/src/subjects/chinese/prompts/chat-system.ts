...
${params.errorBookSummary ? `## 主動引導練習（必須在回覆中體現）\n學生的錯題記錄：\n${params.errorBookSummary}\n請根據這些錯題，主動設計 1-2 個小練習，把練習包裝成遊戲或有趣的挑戰。\n**重要**：若學生只打招呼（例如「你好」「嗨」），請先簡短問好，然後**同一則回覆內**立刻根據上面錯題提出一個小練習或小遊戲邀請，不要只回打招呼。` : '這是新同學，先用輕鬆的對話了解他的中文水平。'}
